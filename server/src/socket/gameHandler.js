...
function endQuestion(io, roomCode, activeRooms) {
  ...
  setTimeout(() => {
    ...
    // FIX: send personal rank keyed by userId from the leaderboard entry,
    // not by displayName (which breaks when two users share a name).
    leaderboard.forEach(entry => {
      if (entry.socketId) {
...
async function finishQuiz(io, roomCode, activeRooms) {
  ...
  try {
    // FIX: single-pass O(answers) grouping — previously this was O(participants × answers)
    // because the inner loop iterated ALL answers for every participant.
  ...
  io.to(roomCode).emit('room:finished', {
    ...
    totalQuestions: activeRoom.quiz.questions.length,
  });

  // Clean up memory after a grace period so late arrivals can still
  // fetch results via the REST endpoint
